...
- (id)initWithStyle:(UITableViewStyle)style {
    ...
    if ((self = [super initWithStyle:style])) {
		...
		s= [[pendingXML alloc]init];
		if([s.xmlobj1 count]== 0)
		{
		
			UILabel *lb1 = [[ UILabel alloc]initWithFrame:CGRectMake(10, 50, 300, 50)];
			lb1.text = @"There is no any  Notificatios for u!!!!";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	pendingdata   *mydata = [s.xmlobj1 objectAtIndex:indexPath.row];
	
	cell.textLabel.text = mydata.loginname;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	pendingDetails  *detailViewController = [[pendingDetails alloc] init];
	// ...
	// Pass the selected object to the new view controller.
	[self.navigationController pushViewController:detailViewController animated:YES];
	[app release];
